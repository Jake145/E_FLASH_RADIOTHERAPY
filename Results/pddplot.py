...
if __name__ == "__main__":
    ...
    path="C:/Users/pensa/Desktop/E_FLASH_RADIOTHERAPY/Flash_ex_novo/VALIDATION/7mevNOVAC_10000000_4cm_100bin_10000000.csv"#seed 42
    ...
    x=np.divide(dose/evts,(Run2_simulated_energy* 1.6e-10/mass_lyso)/2e6)
    coeffs=np.array(x)
    toc=time.perf_counter_ns()
    print("Obtained coefficients: ",coeffs,"time elapsed: ", toc-tic, "nanoseconds")
    ...
    plt.figure(1)
    plt.plot(distance,100*dose_/np.max(dose_),marker='.',linestyle='dashed',color='blue',label='Simulated Water 7 MeV')
    ...
    plt.scatter(validation_distance,validation_dose,marker='.',color='red',label='R100, R90, R50 Water Validation')
    ...
    path_7_1000="C:/Users/pensa/Desktop/Send/darklyso/pmmalysodarkpdd.csv"
    path_7_54545454="C:/Users/pensa/Desktop/Send/darklyso/pmmapdddarklyso2.csv"
    ...
    path_7_1000="C:/Users/pensa/Desktop/Send/darklyso/pmmalysodarkpdd.csv"
    path_7_54545454="C:/Users/pensa/Desktop/Send/darklyso/pmmalysodarkpdd2.csv"
    ...
    simulated_energy_7=np.array([3.74886,3.68112,3.13143,3.01489,2.13634,0.95427]) #lyso seed 42
    simulated_energy_7_seed_106=np.array([4.00161,3.7975,3.21129,3.25056,1.9335,0.892416])
    simulated_energy_7_seed_145=np.array([3.68242,3.77736,3.38892,3.05226,1.98661,0.908911])

    simulated_energy_7=mean_array_calculator(simulated_energy_7,simulated_energy_7_seed_106,simulated_energy_7_seed_145)
...
#seed 42
    ...
    ej212_energy_sim_3=np.array([9.84701,10.0883,10.331,10.5256,10.5953,10.4832,10.1652,9.11255,6.69084,4.84278,2.12886,0.436584])#gev (starts from 5mm
#seed 145
    ej_212_energy_sim_145=np.array([9.95242,10.0633,10.458,10.4576,10.508,10.574,10.1423,8.96335,6.74539,4.97139,2.09613,0.439371])
#seed 111555875875
    ej_212_energy_sim_lol=np.array([9.95562,10.1578,10.3613,10.3675,10.5164,10.4649,10.4815,8.97829,6.60971,4.80155,2.12145,0.42088])
#seed 8675309
    ej_212_energy_sim_jenny=np.array([9.76461,10.0794,10.5218,10.4084,10.4092,10.4342,10.424,8.94853,6.60155,4.76539,2.12755,0.412595])
#seed 106
    ej_212_energy_sim_106=np.array([9.79503,10.0259,10.2793,10.4171,10.5105,10.5316,10.2792,9.04665,6.72886,4.8282,2.0578,0.420623])


    ej212_energy_sim=mean_array_calculator(ej212_energy_sim_3,ej_212_energy_sim_145,ej_212_energy_sim_lol,ej_212_energy_sim_jenny,ej_212_energy_sim_106)
    ...
    ej212_measured_charge_first=np.array([7.220,7.360,7.510,7.580,7.610,7.490,7.300,6.360,4.450,2.930,0.580,0.480])

    ej212_measured_charge_second=np.array([7.210,7.310,7.460,7.530,7.550,7.460,7.240,6.370,4.460,2.960,0.620,0.480])

    ej212_measured_charge_third=np.array([7.150,7.260,7.420,7.480,7.520,7.400,7.210,6.330,4.480,2.980,0.580,0.460])


    ej212_measured_charge=mean_array_calculator(ej212_measured_charge_first,ej212_measured_charge_second,ej212_measured_charge_third) - ej212_noise
...
##9Mev_ej212 EF medium-equivalence coefficients
    ...
    toc=time.perf_counter_ns()
    print("Obtained coefficients: ",coefficients_ej212,"time elapsed: ", toc-tic, "nanoseconds")
    ...
    plt.figure("ej212")
    plt.plot(distance_9,100*unumpy.nominal_values(dose_9)/max(unumpy.nominal_values(dose_9)),marker='.',linestyle='dashed',color='blue',label='Simulated PMMA 9 MeV ')
    plt.plot(ej212_distance,100*unumpy.nominal_values(ej212_energy_sim)/max(unumpy.nominal_values(ej212_energy_sim)),marker='.',linestyle='dashed',color='magenta',label='Simulated EJ212 9 MeV ')
    plt.errorbar(ej212_distance,unumpy.nominal_values(100*(ej212_charge_over_pulse/max(ej212_charge_over_pulse_means))),yerr=unumpy.std_devs(100*(ej212_charge_over_pulse/max(ej212_charge_over_pulse_means))),marker='.',linestyle='dashed',color='black',label='Measured EJ212 9 MeV ')



    plt.plot(validation_distance_9,validation_dose_9,marker='.',linestyle='dashed',color='red',label='Measured PMMA')

    plt.errorbar(ej212_distance,100*unumpy.nominal_values((c_corr_ej/max(c_corr_ej_means))),yerr=100*unumpy.std_devs((c_corr_ej/max(c_corr_ej_means))),marker='.',linestyle='dashed',color='green',label='Corrected EJ212 ')
...
## 9 Mev LYSO plot
    ...
    plt.figure(2)
    plt.plot(distance_9,100*unumpy.nominal_values(dose_9)/max(unumpy.nominal_values(dose_9)),marker='.',linestyle='dashed',color='blue',label='pmma simulato 9 MeV ')
    ...
    plt.scatter(unumpy.nominal_values(x_r),unumpy.nominal_values(y_r),marker='.',color='magenta',label='Water Equivalence corrected Points')
...
## Dark spectrum coefficients
    ...
    toc=time.perf_counter_ns()
    print("Obtained coefficients: ",coefficients_lyso,"time elapsed: ", toc-tic, "nanoseconds")
...
## Dark spectrum measurements and PDD plot
    ...
    measured_charge=mean_array_calculator(measured_charge_lyso_1,measured_charge_lyso_2,measured_charge_lyso_3,measured_charge_lyso_4,measured_charge_lyso_5)
    noise=ufloat(np.mean([0.310,0.310,0.310,0.310,0.320]),np.std([0.310,0.310,0.310,0.310,0.320]))
    measured_distance=np.array([0,2,5,10,12,17,22])+1.5
    ...
    plt.figure(5)
    plt.plot(distance_9,100*dose_7_means/np.max(dose_7_means),marker='.',linestyle='dashed',color='blue',label='Simulated PMMA  ')
    plt.plot(validation_distance_7,validation_dose_7,marker='.',linestyle='dashed',color='red',label='Measured PMMA validation')

    plt.errorbar(measured_distance[1:],100*unumpy.nominal_values(charge_per_pulse)[1:]/np.max(unumpy.nominal_values(charge_per_pulse)[1:]),yerr=unumpy.std_devs(100*charge_per_pulse[1:]/max(charge_per_pulse[1:])),marker='.',linestyle='--',color='black',label='Measured LYSO')

    plt.plot(exp_distance_9,100*unumpy.nominal_values(simulated_energy_7)/np.max(unumpy.nominal_values(simulated_energy_7)),marker='.',linestyle='dashed',color='green',label='Simulated LYSO  ')
    plt.errorbar(x_r,unumpy.nominal_values(y_r),yerr=unumpy.std_devs(y_r),marker='.',linestyle='',color='magenta',label='PMMA Equivalence corrected Points')
...
## Dark spectrum plotting
    x,y=np.loadtxt("C:/Users/pensa/Desktop/Send/efdark.txt.txt",unpack=True)
